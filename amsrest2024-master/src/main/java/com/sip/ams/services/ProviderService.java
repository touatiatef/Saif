...
//	}
	...
	public Provider create(MultipartFile file, String name, String email, String adress) throws IOException {
		// generate new Random image name
				...
				Provider provider = new Provider(name,  email, adress,newImageName);
...
//	
	public Provider update(MultipartFile file, String name, String email, String address, long id) {
		...
		return providerRepository.findById(id).map(provider -> {
			...
			if(file!=null) {
			    newImageName = getSaltString().concat(file.getOriginalFilename()); // copy th new name image
			try {
				Files.copy(file.getInputStream(), this.root.resolve(newImageName)); //add the image in the path 
			} catch (Exception e) {
				throw new RuntimeException("Could not store the file. Error: " + e.getMessage());
			}
			}
